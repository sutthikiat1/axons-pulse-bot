...
// Active check-in window: 16:30 – 19:00 (inclusive of 19:00:00)
const CHECKIN_WINDOW_START_MIN = 16 * 60 + 30; // 16:30
...
async function handleButton(interaction) {
  // Time gate — block clicks outside 16:30–19:00 window
  if (!isWithinCheckinWindow()) {
    return interaction.reply({
      content:
        "❌ ปุ่มใช้งานได้เฉพาะช่วง **16:30 – 19:00 น.** เท่านั้น (Asia/Bangkok)",
      ephemeral: true,
    });
  }
  ...
  //   return interaction.reply({
  //     content:
  //       "❌ ปุ่มนี้ไม่ใช่ broadcast ล่าสุดของวันนี้ — ต้องใช้ปุ่มจาก broadcast ล่าสุดเท่านั้น (รอ broadcast 16:30-17:00 หรือดูข้อความ broadcast ใหม่สุด)",
  //     ephemeral: true,
  //   });
  // }
