...
- **enabled: true** → ทุก broadcast สุ่มเลือก 3 สกินจากพูล `CHALLENGE_SKINS`
  ที่เป็น **ไอคอนขัดกับสี** ครบทั้ง 4 สี Discord (ม่วง/เทา/เขียว/แดง) เช่น
  ✅บนแดง, ❌บนเขียว, ⚠️/🚫/❓/💤 — เพื่อกันการกดตามความเคยชิน
- **การันตี:** ไม่มีปุ่มไหนได้หน้าตาจริงของตัวเอง + **ปุ่ม Done ไม่มีทาง
  เป็นเขียว และไม่มีทาง ✅** (`doneLooksSafe`) — muscle memory กดถูกไม่ได้
- Discord มีสีปุ่มแค่ 4 สีเท่านั้น (เพิ่มสีใหม่ไม่ได้) — ความสับสนมาจากไอคอน
...
`CHALLENGE_SKINS` + `assignChallengeSkins()` (สุ่ม 3 อัน + guard ทุก broadcast)
...
1. **Cron** ใน [src/index.js](src/index.js):
   ```js
   cron.schedule("30 16 * * 1-5", ...)  // broadcast: 16:30 + random 0-30 min (BROADCAST_JITTER_MIN)
...
2. **Time gate window** ใน [src/interactions.js](src/interactions.js):
   ```js
   const CHECKIN_WINDOW_START_MIN = 16 * 60 + 30;  // = earliest possible broadcast time
